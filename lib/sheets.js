 ...
 *  - Ventas:      cabeceras de proformas/facturas
...
async function llamar(action, payload = {}) {
  ...
  try {
    res = await fetch(URL_SCRIPT(), {
      ...
      redirect: "follow",
      // CRÍTICO: Next.js cachea automáticamente cualquier fetch() del lado
      // del servidor salvo que se indique lo contrario — incluso dentro de
      // funciones internas como esta. Sin esto, los datos de la hoja podían
      // quedar "congelados" en una versión vieja después de guardar cambios.
      cache: "no-store",
    });
  } catch (e) {
    throw new Error(
...
/**
 * Lee varias pestañas en una sola llamada de red (mucho más rápido que
 * leerTabla() varias veces: cada llamada a Apps Script tiene latencia
 * propia de 1-3s, así que combinarlas ahorra segundos reales de carga).
 * Devuelve un objeto { NombreTabla: [filas...] }.
 */
export async function leerTablas(tablas) {
  const data = await llamar("readMany", { tables: tablas });
  return data.tablas || {};
}
