...
export async function fetchJSON(url, options = {}, timeoutMs = 25000) {
  ...
  // Rompe cualquier capa de caché (navegador, CDN, etc.) agregando un valor
  // único a la URL en cada petición de lectura — así ninguna capa intermedia
  // puede "creer" que ya tiene la respuesta y devolver algo desactualizado.
  const urlFinal =
    metodo === "GET"
  ...
  let res;
  try {
    // cache: "no-store" por defecto — los datos del negocio (stock, precios,
    // pedidos) cambian todo el tiempo y nunca deben servirse desde una copia
    // vieja guardada por el navegador.
    res = await fetch(urlFinal, { cache: "no-store", ...options, signal: controlador.signal });
  } catch (e) {
